#compare average absolute relative biases (AARBs) of true, naive, RCE and AMLE based on 
#weibull regression with both measurement error and misclassification
###This program can reproduce simulation results in Table 5 of main text
###author: Zhiqiang CAO
...
cho_set = 1:10 #scenarios of \beta_m^2\tau_m, measurment error
scen_set = 1:6 #cases of misclassification
final_res = NULL
for (choice in cho_set){ 
  if(choice == 1){
  ...
  for(case in scen_set){ #misclassification cases
    ...
    betae1 = betae2 = betae3 = betae4 = matrix(0, N, 6)
    
    i = k = 1;
    while (i<=N) {
      cat("choice=",choice,"case=",case,"iter=",i,"\n")
      set.seed(seed0+k)
      ...
      if(cc1==TRUE){
        ...
        cond3f = max(var_temp3)>varfac
        #variance of ALE
        var_temp4 = var_amle4_mis(est_temp4, t, delta, m, zo, tau2, c11, c12, c21, c22, p, n)
        var_temp4 = diag(var_temp4)
        cond4f = is.nan(var_temp4[1])|is.nan(var_temp4[2])|is.nan(var_temp4[3])|is.nan(var_temp4[4])|is.nan(var_temp4[5])|is.nan(var_temp4[6])|is.nan(var_temp4[7])|any(var_temp3<0)|max(var_temp4)>varfac
        cc2 = !cond1f & !cond2f & !cond3f & !cond4f 
        if(cc2 == TRUE){
          #naive estimation
    ...
    bias4 = ebetae4 - trubet
    rb4 = (bias4/trubet)*100  #relative bias (%)
    resu4 = rb4[c(2:3,5:6)]   #consider \beta_{ij} (i = 1; 2, j = 0; 1)
    
    resut = rbind(resu2, resu1, resu3, resu4)
    colnames(resut) = c("RB10", "RB20", "RB11", "RB21")
    rownames(resut) = c("true","naive","rce","amle")
  ...
  rownames(choice_res) = c("true","naive","rce","amle")
  final_res = rbind(final_res,choice_res)
}
#final results in Table 5
print(round(final_res,1))
